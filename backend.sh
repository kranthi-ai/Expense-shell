...
then
useradd expense
VALIDATE $? "Adding expense user"  &>>$LOG_FILE_NAME
else
...
cd /app
rm -rf /app/*   #* means everthing from app and removing the code
...
mysql -h mysql.kkdevops.site -uroot -pExpenseApp@1 < /app/schema/backend.sql
